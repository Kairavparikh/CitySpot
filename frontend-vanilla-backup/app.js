...
// Color mapping — updated live by color pickers
const DEFAULT_COLORS = { small: '#4ECDC4', medium: '#FFB84D', large: '#E74C3C' };
const SIZE_COLORS = { ...DEFAULT_COLORS };
...
function setupEventListeners() {
    ...
    ['small', 'medium', 'large'].forEach(size => {
        const picker = document.getElementById(`color-${size}`);
        if (picker) {
            picker.addEventListener('input', (e) => {
                SIZE_COLORS[size] = e.target.value;
                updateSizeButtonColor(size, e.target.value);
                renderParkingLots();
            });
        }
    });
    ...
    if (resetBtn) {
        resetBtn.addEventListener('click', () => {
            ['small', 'medium', 'large'].forEach(size => {
                SIZE_COLORS[size] = DEFAULT_COLORS[size];
                const picker = document.getElementById(`color-${size}`);
                if (picker) picker.value = DEFAULT_COLORS[size];
                updateSizeButtonColor(size, DEFAULT_COLORS[size]);
            });
            renderParkingLots();
        });
    }
...
// Keep size-filter button indicators in sync with chosen color
function updateSizeButtonColor(size, color) {
    const btn = document.querySelector(`.size-btn[data-size="${size}"] .size-indicator`);
